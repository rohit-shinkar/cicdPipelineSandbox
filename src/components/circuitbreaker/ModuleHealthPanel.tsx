...
import { ModuleHealth } from '../../types/circuitBreaker';
import { AlertTriangle, Clock, ChevronDown, ChevronUp } from 'lucide-react';
import { format } from 'date-fns';

interface ModuleHealthPanelProps {
  modules: ModuleHealth[];
}

export const ModuleHealthPanel: React.FC<ModuleHealthPanelProps> = ({ modules }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-lg font-semibold mb-6">Module Health</h2>
      <div className="space-y-8">
                ...
                >
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-medium text-red-600">{module.errorRate}%</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-gray-600">Failures</span>
                      <span className="font-medium">{module.failureCount}</span>
                    </div>
                    <div className="flex items-center text-gray-500">
                      <Clock className="w-3.5 h-3.5 mr-1" />
                      <span>{format(new Date(module.lastStateChange), 'HH:mm:ss')}</span>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {nonClosedModules.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            All modules are in closed state
          </div>
        )}
      </div>
    </div>
